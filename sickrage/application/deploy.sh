...
app_name="sickrage"
app_git_url="https://github.com/SickRage/SickRage.git"
app_destdir="/opt/sickrage"

## First things first, stop application
/usr/bin/supervisorctl stop $app_name
...
deploy_lxcmedia_user()
{
	...
	if [[ -f /proc/syno_cpu_arch ]]; then
		...
		if [[ -z $lkxcuser ]]; then export lxcuser="lkxcmedia"; fi
